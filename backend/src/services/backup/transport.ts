...
/**
 * До хранилища не достучались — сбой сети, а не ответ сервера.
 *
 * Отдельный тип нужен прогону бэкапа: такую ошибку бессмысленно ловить
 * пообъектно и идти дальше. Если канал лежит, следующие сто объектов
 * потратят по несколько минут каждый и превратят прогон в многочасовой,
 * с тем же исходом. Поэтому прогон обрывается на первой такой ошибке.
 */
